module Box
  ...
  def self.admin_token
    access_token = Rails.cache.fetch("/staging/box_tokens/enterprise/#{ENV['BOX_ENTERPRISE_ID']}", :expires_in => TOKEN_TTL) do
  ...
  def self.user_token(user_id)
    access_token = Rails.cache.fetch("/staging/box_tokens/user/#{user_id}", :expires_in => TOKEN_TTL) do
