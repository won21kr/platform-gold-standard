...
def show
    ...
    @product_catalog = Rails.cache.fetch("/staging/product_catalog_folder/#{ENV['PRODUCT_CATALOG_FOLDER']}", :expires_in => 20.minutes) do
      client.folder_from_id(ENV['PRODUCT_CATALOG_FOLDER'], fields: [:id, :name, :size])
    end
    #
    # @files = client.folder_items(@myFolder, fields: [:name, :id]).files
  ...
  def thumbnail
    ...
    image = Rails.cache.fetch("staging/image_thumbnail/#{params[:id]}", :expires_in => 10.minutes) do
      puts "miss!"
